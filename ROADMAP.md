...
### Markdown Tier 2/3（リンク・画像・コード・テーブル）

- **Tier 2 リンク** `[text](url)` → `<a href="...">` を有効化。`validateLink` により `javascript:` / `vbscript:` / 非画像 `data:` スキームを無害化（FR-007）。
- **Tier 2 画像** `![alt](src)` → `<img>` を有効化。同じく `validateLink` で危険スキームを排除。
- **Tier 3 インラインコード** `` `code` `` → `<code>` を有効化。
- **Tier 3 コードブロック** ` ``` ` フェンス → `<pre><code>` を有効化。
- **Tier 3 テーブル** `| 列 | 列 |` → `<table>` を有効化。
- **縦書き破綻回避**: `pre` / `table` は `.tategaki` 内で `writing-mode: horizontal-tb` の横組み島として描画（本文の縦書きは維持）。
- **strikethrough は無効のまま**（Tier 定義外）。
...
- **Tier 3 Markdown 記法の縦書き最適化**: コードブロック・表は横組み島として実装済み。より精緻な組版（コードの折り返し・表の幅制御）は追加対応候補
