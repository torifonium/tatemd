...
配信は 2 経路（design「配信経路」参照）。**静的 Web アプリ**は `window.print()` ベースで即時・無インストール（モード②はブラウザのベスト努力）。**忠実な絵巻 PDF**（実テキスト・確実に 1 枚）は任意の **CLI（Puppeteer）** で提供し、Web と同じ core/CSS を再利用する。どちらも原稿を外部送信しない。
