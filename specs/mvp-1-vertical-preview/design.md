...
- LocalStorage 依存箇所（`adapter/storage.ts`）は VSCode では別実装（`workspace`/`globalState`）になるため、永続化は **インターフェースで抽象化**しておくと移植が楽（初版はプレビュー描画に集中し、永続化は後追い）。
- **`.tategaki` は単一所有**: core はラッパを返さない（B-1）。Webview 側が `.preview-pane > .tategaki` を用意し、その innerHTML に core 出力を差し込む（Web と同形）。
- 実装場所: `vscode-extension/`（独立パッケージ）。`renderToTypesettingHtml` を相対 import し **esbuild でバンドル**、`vertical.css` はビルド時に `media/` へコピーして単一ソースを保つ。コマンド `tatemd.openPreview` で Webview を横に開き、`onDidChangeTextDocument` で再描画。CSP＋`enableScripts:false`＋core の `html:false` で安全。
- 配布（Marketplace 公開）はユーザー判断。拡張本体は実装する（ROADMAP から着手へ移行）。
