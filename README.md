...
| ティア | 使い方 | 特徴 | PDF |
|--------|--------|------|-----|
| **Web（静的）** | ブラウザで URL を開くだけ | 一番軽い・即時・無インストール | 絵巻=横長1枚PDF（新タブ）/ ページ分割=ベスト努力 |
...
ヘッダの `A5` / `B6` ボタンで用紙サイズを選び、`印刷` ボタンでブラウザの印刷ダイアログから本文のみを PDF／印刷できる。`長尺` ボタンは絵巻物スタイルの本文を**新しいタブ**に開き、そこから「PDFで保存」（または ⌘P / Ctrl+P）で**横長一枚**の PDF を出力できる（文字は画像化せず選択可能）。原稿が長いほど横に長い一枚になる。より厳密な用紙別ページ分割や、ローカルで確実に得たい忠実版は下記の CLI で出力する。
...
> `.html` を直接渡すこともできる。`puppeteer` / `@vivliostyle/cli` は任意依存（`optionalDependencies`。必要な形式のときだけ導入）。
